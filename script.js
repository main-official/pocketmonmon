...
(() => {
  ...
  // Instead of snapping between random points, the fish holds a smoothly
  // drifting heading (like a boat's rudder easing over, not teleporting) and
  // gently steers away from the tank walls before it ever gets close to them.
  const auto = {
    heading: Math.random() * Math.PI * 2,
    wanderAngle: 0,      // slow random drift added to the heading
    speed: 0.045,         // px/ms cruising speed
  };
  ...
  function updateAutoSwim(dt) {
    // slow, smooth random drift in the desired heading (Perlin-ish via sine noise)
    auto.wanderAngle += (Math.random() - 0.5) * 0.06;
    auto.wanderAngle = Math.max(-1.1, Math.min(1.1, auto.wanderAngle));
    let desired = auto.heading + auto.wanderAngle * 0.02;

    // steer gently away from the tank edges well before hitting them
    const margin = 90;
    const minY = waterY + 30, maxY = H - 20;
    let avoidX = 0, avoidY = 0;
    if (fish.x < margin) avoidX = (margin - fish.x) / margin;
    if (fish.x > W - margin) avoidX = -((fish.x - (W - margin)) / margin);
    if (fish.y < minY + margin * 0.6) avoidY = ((minY + margin * 0.6) - fish.y) / margin;
    if (fish.y > maxY - margin * 0.6) avoidY = -((fish.y - (maxY - margin * 0.6)) / margin);
    if (avoidX !== 0 || avoidY !== 0) {
      const avoidHeading = Math.atan2(avoidY, avoidX || 0.0001);
      desired = auto.heading + angleDiff(auto.heading, avoidHeading) * 0.9;
    }

    // ease the actual heading toward the desired heading — no sudden snaps
    const turnRate = 0.0022 * dt;
    auto.heading += angleDiff(auto.heading, desired) * Math.min(1, turnRate);

    // gentle speed breathing so cruising doesn't feel robotic
    const speed = auto.speed * (0.85 + 0.15 * Math.sin(performance.now() * 0.0006));

    fish.vx = Math.cos(auto.heading) * speed;
    fish.vy = Math.sin(auto.heading) * speed;
    fish.x += fish.vx * dt;
    fish.y += fish.vy * dt;

    // hard safety clamp (shouldn't normally trigger thanks to avoidance above)
    if (fish.x < 14) { fish.x = 14; auto.heading = 0; }
    if (fish.x > W - 14) { fish.x = W - 14; auto.heading = Math.PI; }
    if (fish.y < minY) { fish.y = minY; auto.heading = Math.PI / 2; }
    if (fish.y > maxY) { fish.y = maxY; auto.heading = -Math.PI / 2; }

    fish.angle = auto.heading;
    target.x = fish.x; target.y = fish.y; // keep target in sync for a seamless handoff to dragging
  }
  ...
  function onPointerUp(e) {
    ...
    autoResumeDelay = 700; // let the fish coast a moment before it starts wandering again
    // pick up wandering in whatever direction it was already headed — no snap
    if (Math.hypot(fish.vx, fish.vy) > 0.02) {
      auto.heading = Math.atan2(fish.vy, fish.vx);
    }
    auto.wanderAngle = 0;
  ...
  function confirmName() {
    ...
    auto.heading = Math.random() * Math.PI * 2;
    auto.wanderAngle = 0;
    autoResumeDelay = 0;
  ...
  function frame(now) {
    ...
    if (phase === "egg") {
      ...
      if (fish.mode === "jump") {
        ...
        // brief coast right after letting go — keeps moving the way it was already going
        autoResumeDelay -= dt;
        ...
        // no one is touching the screen — swim around on its own, smoothly
        updateAutoSwim(dt);
      }
      drawFish(now, dt);
    }
