...
(() => {
  ...
  function updateJump(now) {
    ...
    if (t >= 1) {
      fish.mode = "swim";
      fish.jump = null;
      // Clear the stale jump velocity so the post-landing swim easing
      // starts from rest instead of inheriting the leap's steep vy/vx —
      // without this the fish could visibly "pop" or keep a tilted nose
      // for a frame right as it hits the water.
      fish.vx = 0;
      fish.vy = 0;
  ...
  infoConfirm.addEventListener("click", () => infoOverlay.classList.add("hidden"));
    ...
    if (phase === "egg") {
      ...
      if (fish.mode === "jump") {
        ...
        if (pointer.dragging) {
          ...
          velEase = 0.10;   // gentle acceleration — no instant velocity snaps
          turnRate = 0.0022; // gentler heading changes while idle-wandering
        }

        // Ease the *velocity* toward the desired velocity instead of
        // snapping straight to it. This is what keeps the fish from
        // visibly "teleporting" its direction the moment a new wander
        // target is picked — it curves into the new heading instead.
        const desiredVX = (target.x - fish.x) * ease;
        const desiredVY = (target.y - fish.y) * ease;
        fish.vx += (desiredVX - fish.vx) * velEase;
        fish.vy += (desiredVY - fish.vy) * velEase;
        ...
        // Turn the body toward the direction of travel at a capped rate
        // rather than snapping the heading every frame. When the fish is
        // nearly stationary (most notably right after landing from a
        // jump) relax the heading back toward level instead of freezing
        // at whatever steep angle it last had — this is what fixed the
        // "stuck nose-down" glitch after a flip.
        const spd = Math.hypot(fish.vx, fish.vy);
        const desiredAngle = spd > 0.08
          ? Math.atan2(fish.vy, fish.vx)
          : (Math.cos(fish.angle) >= 0 ? 0 : Math.PI);
        let diff = desiredAngle - fish.angle;
        diff = Math.atan2(Math.sin(diff), Math.cos(diff)); // shortest signed angle
        const maxTurn = turnRate * dt;
        fish.angle += Math.max(-maxTurn, Math.min(maxTurn, diff));
      }
      drawFish(now, dt);
    }
